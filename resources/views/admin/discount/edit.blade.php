...
@section('content')
    <div class="middle"><!-- start middle -->
        ...
        <form action="{{ route('discount.update',['id'=>$dis->id]) }}" method="post" class="text-right mr-5 mb-3">
            {{csrf_field()}}
            {{method_field('PATCH')}}
            ...
            value="{{ $dis->name}}" placeholder="نام مربوط به تخفیف را وارد کنید">
            

            <input type="text" class="form-control ml-2" id="percent" name="percent"
            value="{{ $dis->percent }}" placeholder=" درصد تخفیف را وارد کنید">
            </div>
